...
d1_ab <-subset(d1,srch_destination_id==8286)
a<- d1_ab
a$monthOfYear <- format(as.Date(a$srch_ci), "%Y-%m-1")
b <- sqldf('select monthOfYear,count(*) c from a where srch_destination_type_id > -13 and hotel_cluster > -53 group by monthOfYear')
