...
object PredictAndSaveApp extends LazyLogging {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val distPredictionSeq = (0 until userLocMarketList.rows).map { r =>
      ...
      val predOutputFile = "c:/perforce/daniel/ex/segments/loc_market_dest/more_than_1000/predictions/predicted_clusters_loc_%d_market_%d_dest_%d.csv.csv".format(userLoc, marketId, destId)

      if (destSize < 5000 && !(new File(predOutputFile).exists())) {

        val trainDS = ExCSVDataSource(dsName = "trainDS", "c:/perforce/daniel/ex/segments/loc_market_dest/more_than_1000/train_2013_loc_%d_market_%d_dest_%d.csv".format(userLoc, marketId, destId))
        val trainClicks = trainDS.getAllClicks()

        val testDS = ExCSVDataSource(dsName = "testDS", "c:/perforce/daniel/ex/segments/loc_market_dest/more_than_1000/train_2014_booked_only_loc_%d_market_%d_dest_%d.csv".format(userLoc, marketId, destId))
