 ...
 userLocMarketList <- sqldf('select user_location_city,hotel_market,count(*) from train_13 where is_booking > -1 group by user_location_city,hotel_market having count(*) > 1000 order by count(*)')
 write.csv(userLocMarketList,'svm/svm_dist1000/userLocMarketList.csv',row.names=F) 
  ...
  for(i in 1: nrow(userLocMarketList)) {
    ...
    write.csv(svmProbs,sprintf('svm/svm_dist1000/svm_predictions_loc_%d_market_%d.csv',userLoc,market),row.names=F)
  }
