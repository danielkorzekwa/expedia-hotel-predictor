...
case class CompoundHyperParams(model: String, hyperParamsByMarket: mutable.Map[Int, SimpleHyperParams], hyperParamsByCountry: mutable.Map[Int, SimpleHyperParams]) {

  def getParamValueForCountry(param: String, countryId: Int): Double = {
    hyperParamsByCountry.getOrElseUpdate(countryId, SimpleHyperParams(getDefaultParams(model))).getParamValue(param)
  }
  def getParamValueForMarketId(param:String,marketId:Int):Double = {
    hyperParamsByMarket.getOrElseUpdate(marketId, SimpleHyperParams(getDefaultParams(model))).getParamValue(param)
  }
