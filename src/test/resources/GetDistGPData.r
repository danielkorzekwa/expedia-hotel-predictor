...
userLocMarketList <- sqldf('select user_location_city,hotel_market,srch_destination_id, count(*) c from train_13 group by user_location_city,hotel_market,srch_destination_id having count(*)>900 and count(*)<1200 order by count(*) desc limit 100')
